...
## 使用您自己的 clientid 与 secret

如果您的账户没有管理权限，那么您就无法使用公共的 `clientId` 与 `clientSecret` 。这个情况下，您就要生成您自己的 client id 与 secret 了。
...
1. 登入您的微软账户，点击 _New registration_ 。
...
### 获取您的 client id 与 secret

您的应用 (client) ID 就是 `api.config.js` 里的 `clientId` ，它应该出现在 _Overview_ > _Essentials_ 。

![Your application (client) ID](./_images/advanced/get-client-id.png)

您的 client secret 需要手动获取
...
<Callout>现在，您就准备好您自己的 `clientId` 与 `clientSecret` 了。</Callout>

### 修改 `api.config.js`

您可以直接将 `clientId` 修改为您自己的 `clientId` 。

但是，client secret 需要保密，您需要在下面进行 AES 加密：

<Encryption />

填写 client secret 后，您应该得到一个长得像这样的字符串：
...
如果您修改了 `redirectUri` 为其他值，您也要在 `api.config.js` 里设置好。
